 ...
 */
template <typename T> std::span<const byte> GetAsByteSpan(const T& data)
{
 ...
 */
static inline ComPtr<ID3D12Resource> CreateTexture2D(ID3D12Device* pDev, UINT width, UINT height, DXGI_FORMAT format,
                                                     LPCWSTR name, D3D12_RESOURCE_STATES initialState,
                                                     UINT mipLevels = 1)
{
    ...
    ThrowIfFailed(pDev->CreateCommittedResource(&heapProps, D3D12_HEAP_FLAG_NONE, &desc, initialState, nullptr,
...
static inline void EnqueueBarrier(ID3D12Resource* pRes, D3D12_RESOURCE_STATES before, D3D12_RESOURCE_STATES after,
                                  std::span<D3D12_RESOURCE_BARRIER> barriers, int& bCount, UINT subresource = 0)
{
    if (!pRes)
        return;
 ...
 */
static inline void AddBarrier(ID3D12GraphicsCommandList* cmdList, ID3D12Resource* pRes, D3D12_RESOURCE_STATES before,
                              D3D12_RESOURCE_STATES after)
{
 ...
 */
static inline void CreateSRVs(ID3D12Device* pDev, FrameDescriptorHeap& heap, std::span<ID3D12Resource* const> inputs,
                              std::span<const MipChainDesc> mips = {})
{
    if (!mips.empty())
    {

...
class StagingProbe
{
  public:
    ...
    ~StagingProbe()
    {
        for (auto& s : captures_)
            FreeSlot(s);
        for (auto& s : outputs_)
            FreeSlot(s);
    }

    RecordResult Record(Mode mode, ID3D12Device* device, ID3D12GraphicsCommandList* cmd, ID3D12Resource* color,
                        D3D12_RESOURCE_STATES colorState, UINT validWidth, UINT validHeight, uint64_t evaluateSequence)
    {
        ...
        if (desc.Format != DXGI_FORMAT_R16G16B16A16_FLOAT && desc.Format != DXGI_FORMAT_R8G8B8A8_UNORM &&
            ...
            desc.Format != DXGI_FORMAT_B8G8R8A8_UNORM_SRGB && desc.Format != DXGI_FORMAT_R10G10B10A2_UNORM &&
            desc.Format != DXGI_FORMAT_R11G11B10_FLOAT && desc.Format != DXGI_FORMAT_R16G16B16A16_UNORM)
        ...
        if (colorState != D3D12_RESOURCE_STATE_COPY_SOURCE)
        {
            ...
            b[nBarriers].Transition = { color, D3D12_RESOURCE_BARRIER_ALL_SUBRESOURCES, colorState,
                                        D3D12_RESOURCE_STATE_COPY_SOURCE };
            ++nBarriers;
        }
        b[nBarriers].Type = D3D12_RESOURCE_BARRIER_TYPE_TRANSITION;
        b[nBarriers].Transition = { cap.texture, D3D12_RESOURCE_BARRIER_ALL_SUBRESOURCES, cap.currentState,
        ...
        {
            b[nBarriers].Type = D3D12_RESOURCE_BARRIER_TYPE_TRANSITION;
            b[nBarriers].Transition = { source->texture, D3D12_RESOURCE_BARRIER_ALL_SUBRESOURCES, source->currentState,
                                        D3D12_RESOURCE_STATE_COPY_SOURCE };
            ++nBarriers;
        }
        b[nBarriers].Type = D3D12_RESOURCE_BARRIER_TYPE_TRANSITION;
        b[nBarriers].Transition = { out.texture, D3D12_RESOURCE_BARRIER_ALL_SUBRESOURCES, out.currentState,
                                    D3D12_RESOURCE_STATE_COPY_DEST };
        ++nBarriers;
        cmd->ResourceBarrier(nBarriers, b);
  ...
  private:
    ...
    bool EnsureSlot(ID3D12Device* dev, Slot& slot, UINT w, UINT h, DXGI_FORMAT fmt)
    {
        ...
        if (FAILED(dev->CreateCommittedResource(&heap, D3D12_HEAP_FLAG_NONE, &td, D3D12_RESOURCE_STATE_COMMON, nullptr,

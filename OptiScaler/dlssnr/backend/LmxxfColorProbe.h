...
inline Mode ParseMode(std::string_view value)
{
    ...
    if (value == "copy-current")
        return Mode::CopyCurrent;
    if (value == "staging-current")
        return Mode::StagingCurrent;
    if (value == "staging-previous")
        return Mode::StagingPrevious;
    if (value == "proxy-original")
        return Mode::ProxyOriginal;
    if (value == "split-original")
        return Mode::SplitOriginal;
    if (value == "codec-passthrough")
        return Mode::CodecPassthrough;
...
inline Evidence& CurrentEvidence()
{
    static thread_local Evidence e;
    return e;
}
...
class ColorCopy
{
  ...
  public:
    ...
    ID3D12Resource* Record(ID3D12Device* device, ID3D12GraphicsCommandList* cmd, ID3D12Resource* color,
                           D3D12_RESOURCE_STATES state, UINT validWidth, UINT validHeight)
    {
        ...
        if (d.Dimension != D3D12_RESOURCE_DIMENSION_TEXTURE2D || d.Format != DXGI_FORMAT_R16G16B16A16_FLOAT ||
            ...
            d.Height > D3D12_REQ_TEXTURE2D_U_OR_V_DIMENSION || (d.Flags & D3D12_RESOURCE_FLAG_ALLOW_DEPTH_STENCIL))
            return nullptr;

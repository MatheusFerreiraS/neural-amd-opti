...
class CommandListProxy final : public ID3D12GraphicsCommandList10, public ILogicalCommandList
{
    ...
    void MarkSplitIneligible(const char* reason)
    {
        ...
        if (!splitIneligibleReason)
        {
            ...
            {
                LOG_WARN("CommandListProxy {:p} MarkSplitIneligible: {} (event #{})", (void*) this,
                         reason ? reason : "unknown", c);
            }
        }
    }
  ...
  public:
        ...
        *ppv = nullptr;
        ...
        if (auto* cur = logical.Current())
        {
            ...
            if (SUCCEEDED(hr))
            {
                ...
                if (c <= 10 || (c <= 100 && (c % 20 == 0)) || (c <= 1000 && (c % 100 == 0)) || (c % 1000 == 0))
                {
                    LOG_WARN("CommandListProxy {:p} rawInterfaceEscaped: riid={:08X} (event #{})", (void*) this,
                             riid.Data1, c);
                }
            }
            return hr;
    ...
    {
        if (rawInterfaceEscaped)
            return "native_interface_escaped";
        if (logical.WasSplit())
            return "already_split";
        if (splitIneligibleReason)
            return splitIneligibleReason;
        if (!queryBook.CanSplit())
            return "open_query";
    ...
    BOOL STDMETHODCALLTYPE CapturedSamplePositions() override { return contState.hasSamplePositions ? TRUE : FALSE; }
    ...
    void STDMETHODCALLTYPE CopyBufferRegion(ID3D12Resource* d, UINT64 o, ID3D12Resource* s, UINT64 so,
                                            UINT64 n) override
    {
        if (auto* c = Cur())
    ...
    void STDMETHODCALLTYPE AtomicCopyBufferUINT64(ID3D12Resource* dst, UINT64 dstOff, ID3D12Resource* src,
                                                  UINT64 srcOff, UINT deps, ID3D12Resource* const* depRes,
                                                  const D3D12_SUBRESOURCE_RANGE_UINT64* depRanges) override
    {
        if (auto* c = CurAs<ID3D12GraphicsCommandList1>())
        {

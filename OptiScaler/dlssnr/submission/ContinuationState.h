...
struct ContinuationState
{
    ...
    void OnOm(UINT n, const D3D12_CPU_DESCRIPTOR_HANDLE* rt, BOOL single, const D3D12_CPU_DESCRIPTOR_HANDLE* ds)
    {
        numRts = 0;
        hasOm = false;
        hasDsv = false;
    ...
    void ApplyTo(ID3D12GraphicsCommandList* list) const
    {
        ...
        if (hasOm || hasDsv)
            list->OMSetRenderTargets(numRts, hasOm ? rts : nullptr, omSingle, hasDsv ? &dsv : nullptr);
        if (hasIb)

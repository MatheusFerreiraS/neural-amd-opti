...
inline constexpr Bootstrap bootstraps[] {
    { &kAmd0217, 0x8260, 0x6c0c, { 0x4c, 0x8d, 0x05, 0x61, 0x16, 0,    0,    0x31, 0xc9, 0x31,
                                   0xd2, 0x45, 0x31, 0xc9, 0xff, 0x15, 0xe4, 0xa4, 0x07, 0 } },
    { &kAmd03, 0x8400, 0x6cac, { 0x4c, 0x8d, 0x05, 0x61, 0x17, 0,    0,    0x31, 0xc9, 0x31,
...
inline Match MatchBootstrap(const Bootstrap& expected, std::uintptr_t image, std::uintptr_t start,
                            std::uintptr_t returnPc, const unsigned char* call, const unsigned char* entry)
{
...
__declspec(noinline) inline HANDLE WINAPI CreateThreadFiltered(LPSECURITY_ATTRIBUTES attributes, SIZE_T stackSize,
        ...
        else
        {
            ...
            if (entry == image + expected.startRva || returnPc == image + expected.returnRva)
            {
                ...
                const DWORD pathLength =
                    GetModuleFileNameW(static_cast<HMODULE>(caller.AllocationBase), context->modulePath.data(),
                                       static_cast<DWORD>(context->modulePath.size()));
                ...
                const auto* callBytes =
                    reinterpret_cast<const unsigned char*>(image + expected.returnRva - expected.callBytes.size());
                ...
                const auto match = MatchBootstrap(expected, image, entry, returnPc, callReadable ? callBytes : nullptr,
                                                  entryReadable ? entryBytes : nullptr);
                if (entry != image + expected.startRva || returnPc != image + expected.returnRva)
                ...
                context->argumentMask |= (attributes ? 1u : 0u) | (stackSize ? 2u : 0u) | (parameter ? 4u : 0u) |
                                         (flags ? 8u : 0u) | (threadId ? 16u : 0u);
                ...
                context->invalid |= !sameImage || match != Match::Verified || attributes || stackSize || parameter ||
...
inline std::string FailureDetails(const Loading& context, HMODULE module, std::uint64_t callsBefore)
{
    ...
    const auto candidateLength =
        (std::min) (context.candidatePathLength, static_cast<DWORD>(context.modulePath.size() - 1));
    ...
    message << "AMD bootstrap isolation was not verified; initialization stopped, restart required"
            ...
            << " tlsArmed=" << context.tlsArmedCalls << " callerQueryFailed=" << context.callerQueryFailures
            << " callerNotImage=" << context.nonImageCallers << " candidates=" << context.candidates
            ...
            << " suppressed=" << context.suppressedCount << " baseMatch=" << (context.suppressedBase == module)
            << " invalidMask=0x" << std::hex << context.invalidReasons << " argMask=0x" << context.argumentMask
            << " expectedStartRva=0x" << context.expected->startRva << " expectedReturnRva=0x"
            << context.expected->returnRva << " lastStart=0x" << context.lastStart << " lastReturn=0x"
            << context.lastReturn << " candidateStart=0x" << context.candidateStart << " candidateReturn=0x"
            << context.candidateReturn << " candidateBase=0x" << context.candidateBase << " workerBase=0x"
            << context.candidateWorkerBase << " suppressedBase=" << static_cast<const void*>(context.suppressedBase)
            << " loadedBase=" << static_cast<const void*>(module) << " stackSize=0x" << context.candidateStackSize
            << " flags=0x" << context.candidateFlags << std::dec << " candidatePathError=" << context.candidatePathError
            << " loadedPathError=" << loadedPathError << " expectedPath='"
            << Utf8(context.expectedPath, context.expectedPathLength) << "' candidatePath='"
            << Utf8(context.modulePath.data(), static_cast<int>(candidateLength)) << "' loadedPath='"
            << Utf8(loadedPath.data(),
                    static_cast<int>((std::min) (loadedLength, static_cast<DWORD>(loadedPath.size() - 1))))
            << "']";
    return message.str();
}
} // namespace Detail
...
inline HMODULE Load(const wchar_t* path, const AmdLayout* layout)
{
    ...
    HMODULE module =
        LoadLibraryExW(fullPath.c_str(), nullptr, LOAD_LIBRARY_SEARCH_DLL_LOAD_DIR | LOAD_LIBRARY_SEARCH_DEFAULT_DIRS);

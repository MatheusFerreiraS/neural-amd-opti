...
{
    return false; /* Paused after PCSX2 corruption report. */
    static bool target = []
    {
        wchar_t path[MAX_PATH] {};
        GetModuleFileNameW(nullptr, path, MAX_PATH);
        return _wcsicmp(std::filesystem::path(path).filename().c_str(), L"pcsx2-qt.exe") == 0;
    }();
    return target;
}
...
{
    if (FAILED(h))
        throw std::runtime_error("PCSX2 final-image D3D12 failure: " + std::to_string((UINT) h));
}
...
struct Context
{
    ...
    Context(ID3D12Device* d, ID3D12CommandQueue* q) : device(d), queue(q)
    {
        ...
        Check(D3DCompile(shader, strlen(shader), "PCSX2 final compose", nullptr, nullptr, "main", "cs_5_0", 0, 0, &b,
                         &e));
    ...
    void Frame(ID3D12Resource* back, const std::filesystem::path& directory, AmdPreSr::Settings settings)
    {
        ...
        {
            status = "PCSX2 experimental: GPU work pending";
            return;
        }
        ...
        if (!backend)
        {
            ...
            {
                status = "PCSX2 experimental: unsupported backbuffer format";
                return;
            }
            // BGRA typed UAV stores are not universally available.
            if (desc.Format == DXGI_FORMAT_B8G8R8A8_UNORM)
            {
                status = "PCSX2 experimental: BGRA output not supported yet";
                return;
            }
        ...
        {
            status = "PCSX2 experimental: restart emulator after output size change";
            return;
        }
        heldBack = back;
    ...
    void Compose(ID3D12Resource* back, ID3D12Resource* result)
    {
        ...
        Transition(cmd.Get(), back, D3D12_RESOURCE_STATE_PRESENT, D3D12_RESOURCE_STATE_COPY_DEST);
        cmd->CopyResource(back, output.Get());
        Transition(cmd.Get(), back, D3D12_RESOURCE_STATE_COPY_DEST, D3D12_RESOURCE_STATE_PRESENT);
                   ...
                   AmdPreSr::Settings s)
{
    ...
    try
    {
        ...
        Check(sc->GetDevice(IID_PPV_ARGS(&d)));
        if (!context)
            context = new Context(d.Get(), queue);
        if (context->device.Get() != d.Get() || context->queue.Get() != queue)
        {
            status = "PCSX2 experimental: device/queue changed; restart required";
            return;
        }
        ComPtr<ID3D12Resource> b;
        Check(sc->GetBuffer(sc->GetCurrentBackBufferIndex(), IID_PPV_ARGS(&b)));
        context->Frame(b.Get(), directory, s);
    }

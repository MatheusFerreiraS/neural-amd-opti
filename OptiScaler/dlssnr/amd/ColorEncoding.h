...
class ColorEncoding
{
    ...
    {
        if (FAILED(hr))
            throw std::runtime_error("AMD encoding D3D12 error " + std::to_string((UINT) hr));
    }
  ...
  public:
    ColorEncoding(ID3D12Device* d) : device(d)
    {
        ...
        Check(D3DCompile(EncodingShader, sizeof(EncodingShader), "AMD encoding", nullptr, nullptr, "main", "cs_5_0", 0,
                         0, &b, &e));
    ...
    ID3D12Resource* Run(ID3D12GraphicsCommandList* c, ID3D12Resource* src, D3D12_RESOURCE_STATES state, UINT w, UINT h,
                        UINT mode, bool inverse)
    {
        if (!output || output->GetDesc().Width != w || output->GetDesc().Height != h)
        {
        ...
        Barrier(c, src, state, D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE);
        ...
        UINT v[] = { w, h, mode, inverse ? 1u : 0u };
        c->SetComputeRoot32BitConstants(1, 4, v, 0);
        ...
        Barrier(c, src, D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE, state);
        return output.Get();
    }
};
} // namespace AmdPreSr
